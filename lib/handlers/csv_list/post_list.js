    ...
    if(exists) {
      ...
      */
      if(request.payload.listName === 'list') {
        ...
        return reply.view('csvListForm', {errorMessage: 'Sorry "list" is a reserved keyword', listName: request.payload.listName});
      }
      ...
      addListToCandidates(request.payload.csvFile, request.payload.listName, function(parseCsvError){
        ...
        if(parseCsvError) {
          ...
          return reply.view('csvListForm', {errorMessage: 'Sorry wrong format of the file. Is it a csv file? Does it have Name and Email columns?', listName: request.payload.listName});
        }
