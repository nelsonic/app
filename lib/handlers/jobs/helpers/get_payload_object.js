...
/**
* Create a complete job object from a payload
* @param {Object} payload - The payload send when a job is created or updated
* @param {Object} users - The list of users which is used to define the owner of the job
*/

module.exports = function (payload, users) {

  var user = {};
    users.forEach(function (userObj) {

      if (userObj.id.toString() === payload.user.toString()) {
        user.id = userObj.id;
        user.firstName = userObj.names.firstname;
        user.lastName = userObj.names.lastname;
      }
    });
